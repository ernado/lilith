...
func (db *DB) GetChat(ctx context.Context, id int64) (*lilith.Chat, error) {
	q := psql.Select("id", "info", "last_notes_msg_id", "model", "access_hash", "type").
		From("chat").
		Where("id = ?", id)
	...
	err = db.pgx.QueryRow(ctx, sql, args...).Scan(
		&chat.ID, &chat.Info, &chat.LastNotesMsgID, &chat.Model, &chat.AccessHash, &chat.Type,
	)
	if err != nil {
...
func (db *DB) GetChats(ctx context.Context) ([]lilith.Chat, error) {
	q := psql.Select("id", "info", "last_notes_msg_id", "model", "access_hash", "type").From("chat")
	...
	for rows.Next() {
		...
		if err := rows.Scan(&chat.ID, &chat.Info, &chat.LastNotesMsgID, &chat.Model, &chat.AccessHash, &chat.Type); err != nil {
			return nil, errors.Wrap(err, "scan")
		}
